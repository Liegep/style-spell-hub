...
function BloggersPage() {
  ...
  function replaceBlogger(updated: BloggerListItem) {
    setRows((current) =>
      current.map((row) =>
        row.id === updated.id ? { ...updated, progress: updated.progress ?? row.progress } : row,
      ),
    );
    setSelectedBlogger((current) =>
      current?.id === updated.id
        ? { ...updated, progress: updated.progress ?? current.progress }
        : current,
    );
  }
            ...
            >
              <div>
                ...
                <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.3em] text-foreground/50">
                  {blogger.sl_avatar_name || tr("avatar pending")} · UUID{" "}
                  {blogger.sl_avatar_uuid ? tr("connected") : tr("missing")} ·{" "}
                  {blogger.language_preference.toUpperCase()}
                </div>
              ...
              {blogger.account_status === "active" ||
              blogger.account_status === "pending" ||
              blogger.account_status === "blocked" ? (
...
function BloggerDossierModal({
  ...
  async function removeBlogger() {
    if (removeState === "idle") {
      ...
      setMessage(
        "Click delete again to confirm. This keeps history, but removes her from active lists.",
      );
      return;
    }
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[0.95fr_1.35fr]">
          <GlassCard tone="pink" className="p-5">
            ...
            <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50/50 p-4">
              <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-rose-600">
                {tr("Danger zone")}
              </div>
          ...
          <div className="grid content-start gap-6">
            <div className="grid max-w-3xl items-start gap-4 sm:grid-cols-4">
              <MiniStat label={tr("Claims")} value={progress.claims} />
              <MiniStat label={tr("Delivered")} value={progress.delivered} tone="pink" />
              <MiniStat label={tr("Posts")} value={progress.posts} />
              <MiniStat
                label={tr("Pending")}
                value={progress.pending}
                tone={progress.pending > 0 ? "pink" : "plain"}
              />
            </div>
...
function MiniStat({
  ...
  return (
    <GlassCard tone={tone === "pink" ? "pink" : undefined} className="aspect-square p-4">
      <div className="font-display text-3xl">{String(value).padStart(2, "0")}</div>
      <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.28em] text-foreground/50">
        {label}
      </div>
    </GlassCard>
  );
    ...
    <GlassCard className="p-5">
      ...
      <div className="mt-4 space-y-3">
        {hasChildren ? (
          ...
          <div className="font-hand text-3xl text-[var(--brand-magenta)]">{empty}</div>
        )}
      </div>
    </GlassCard>
  );
    ...
    <div className="rounded-2xl border border-foreground/10 bg-background/60 p-3">
      <div className="grid grid-cols-[56px_1fr_auto] items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.25em] text-foreground/45">
            {meta}
          </div>
        </div>
      ...
      {note ? (
        <p className="mt-3 rounded-xl bg-[var(--brand-pink)]/45 px-3 py-2 text-sm text-foreground/70">
          {note}
        </p>
      ) : null}
    </div>
...
function BloggerProgress({ blogger }: { blogger: BloggerListItem }) {
  ...
  const monthProgress =
    monthlyTarget === 0
      ? 100
      : Math.min(100, Math.round((progress.approvedThisMonth / monthlyTarget) * 100));
  ...
  return (
    <div className="min-w-[220px] rounded-2xl border border-foreground/10 bg-background/55 px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        <div className="font-mono text-[9px] uppercase tracking-[0.28em] text-foreground/45">
          {tr("Progress")}
        </div>
        <div
          className={cn(
      ...
      <div className="mt-2 flex flex-wrap gap-2 font-mono text-[8px] uppercase tracking-[0.22em] text-foreground/45">
        <span>
          {progress.approvedThisMonth} {tr("this month")}
        </span>
        <span>·</span>
        <span>
          {progress.pending} {tr("pending")}
        </span>
        {progress.needsRevision > 0 ? (
          <>
            <span>·</span>
            <span>
              {progress.needsRevision} {tr("revision")}
            </span>
          </>
        ) : null}
      </div>
...
function ProgressMetric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl bg-foreground/[0.035] px-2 py-2">
      ...
      <div className="mt-1 font-mono text-[7px] uppercase tracking-[0.22em] text-foreground/40">
        {label}
      </div>
    </div>
  );
